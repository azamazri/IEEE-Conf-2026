...
This paper makes four contributions. First, on \emph{task and protocol}, it formalizes
interaction-free dupe retrieval and a non-circular evaluation with a three-source firewall
(local item accords, externally sourced query accords verified from Fragrantica, and
held-out inspired-by labels), and makes explicit why interaction-based recommenders
(KGAT, LightGCN, KGIN) cannot be compared on this data. Second, on \emph{method}, it
proposes an order-two co-occurrence TF-IDF matcher whose cosine score decomposes into
order-one and order-two terms, paired with faithful-by-construction explanation paths for
a downstream explainability layer. Third, on \emph{finding}, it gives a first
non-circular demonstration that order-two co-occurrence ranks first in a fifteen-method
benchmark, beating all twelve comparators across four paradigms---marginal similarity,
neural embeddings, perceptual-taxonomy transport, and learned models---while the two
proposed variants do not improve on it; the result is stated with honest caveats, namely
that the gain over the TF-IDF baseline is modest ($+0.042$ MRR) yet robust and
significant, that the Sentence-BERT loss is partly attributable to query--product text
asymmetry, and that the moderate learned methods only match rather than fall far below the
baseline. Fourth, on \emph{reproducibility}, it provides a complete, deterministic
benchmark of fifteen methods with paired significance testing and bootstrap confidence
intervals. The remainder of the paper reviews related work, formalizes the problem and the
non-circular firewall, presents the proposed method, describes the experimental setup, and
reports, discusses, and bounds the results before concluding.
...
comparison; we instead lift co-occurrence into the content representation itself and derive
explanation paths directly from the shared accord-pairs that produce the score, so they are
discriminative (weighted by inverse document frequency) and faithful by construction.
...
distributions over it \cite{r7,r8,r9,r9b}; we operationalize this as a tree-Wasserstein
distance over the Edwards wheel and measure---rather than assume---whether it helps. A
...
capacity \cite{r25,r26,r27}. These motivate our parameter-free design. To our knowledge,
the intersection occupied here---interaction-free, non-circular, order-two co-occurrence
for perfume retrieval, used to test the benefit of perceptual, neural, and learned
structure---is unaddressed by this literature.
...
cannot be instantiated; the exclusion is a property of the data, and we state it explicitly
rather than report vacuous numbers. A second caveat concerns the proxy itself: the dupe task
measures \emph{perceptual} match, not user satisfaction. Because a dupe is deliberately
formulated to smell like its target, query--gold accord overlap is high (about $86\%$ of a
dupe's accords appear in the query). This overlap is a genuine signal rather than an
artifact, and it bounds how much any method can improve over the marginal baseline.
...
\end{tikzpicture}
\caption{Order-1 versus order-2 matching. Shaded nodes are accords shared by query $q$
and candidate dupe $p$ (order-1). The bold red edge is a rare accord \emph{pair}
(\textit{amber--cinnamon}) that co-occurs in both (order-2). A distractor may share the
same individual accords yet lack this signature pairing; it is the shared rare pair, not
the shared accords, that lifts the true dupe to the top.}
\label{fig:concept}
\end{figure}
...
\subsection{Methods}
We evaluate fifteen methods (Table~\ref{tab:results}) in three tiers. Tier~1 is
...
\begin{table}[t]
\caption{Methods and retrieval accuracy (mean over 5 seeds for stochastic methods). Best in bold.}
\begin{center}
\footnotesize
\begin{tabular}{llccc}
\toprule
\textbf{ID} & \textbf{Method (paradigm)} & \textbf{MRR} & \textbf{H@1} & \textbf{H@3} \\
\midrule
\textbf{P1} & order-2 co-occ.\ TF-IDF \textit{(proposed)} & \textbf{0.496} & \textbf{0.378} & \textbf{0.562} \\
P2 & P1 $+$ LTR fusion & 0.500 & 0.379 & 0.580 \\
P3 & P1 $+$ hub-discriminative IDF & 0.448 & 0.335 & 0.491 \\
\midrule
B1 & Jaccard (marginal) & 0.428 & 0.303 & 0.487 \\
B2 & TF-IDF cosine (baseline) & 0.454 & 0.335 & 0.507 \\
B3 & BM25 (marginal) & 0.388 & 0.278 & 0.433 \\
B4 & Sentence-BERT (neural) & 0.272 & 0.163 & 0.311 \\
B5 & Word2Vec (neural) & 0.185 & 0.110 & 0.182 \\
B6 & node2vec (neural) & 0.340 & 0.233 & 0.371 \\
A1 & wheel tree-Wasserstein (perceptual) & 0.284 & 0.174 & 0.306 \\
A2 & PPMI $+$ SVD (learned) & 0.444 & 0.343 & 0.498 \\
A3 & signature LTR (learned) & 0.433 & 0.303 & 0.500 \\
A4 & bigram salience (learned) & 0.139 & 0.060 & 0.145 \\
A5 & bilinear metric (learned) & 0.150 & 0.072 & 0.150 \\
A6 & GBM fusion (learned) & 0.457 & 0.325 & 0.533 \\
\bottomrule
\end{tabular}
\label{tab:results}
\end{center}
...
accord \emph{pairs}---not individual accords---lift the correct dupe. The gain cannot be
large by construction (Section~\ref{sec:problem}): with about $86\%$ of a dupe's accords
...
quality, and we refrain from concluding that neural semantics cannot help under a more
...
instance, the gold product is supported by signature pairs such as \textit{amber--cinnamon}
and \textit{cinnamon--warm spicy}. A counterfactual edge-ablation study quantifying their
...
modest. The dupe relation is a \emph{perceptual} proxy, not user satisfaction, and its
high query--gold overlap is intrinsic to the task. The contribution is scoped to a finding
